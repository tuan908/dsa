...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (Delete(hashtable, k) == 0) cout << "KHONG XOA DUOC\n";
    }
...
void CreateHashtable(Hashtable &ht, int m) {
    ht.table = new List[m];
    for (int i = 0; i < m; i++) CreateList(ht.table[i]);
    ht.M = m;
    ht.n = 0;
}

int Hash(Hashtable ht, int maso) { return maso % ht.M; }
